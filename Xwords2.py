...
def fill_in_words(raw_crossword, height, width, words_in_xword, old_finished_horiz_words, old_finished_vert_words, opp_word_set):
    global FIRSTRUN, CT
    CT += 1
    if is_invalid(opp_word_set) and height * width != 225:
        return ''
            ...
            if letter != '-':
                if potential_words_for_choice is None:
                    try:
                        ...
                        if height == 5:
                            return ''
                        if height * width == 225 and len(actual_word) > 10:
                            best_choice = (5, orientation, beginning_pos, actual_word)
                            best_choice_potential_words = all_words_grouped_by_len[len(actual_word)]
                            break
                        else:
                            return ''
